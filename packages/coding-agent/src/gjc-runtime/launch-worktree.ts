...
function installWorkspaceDependencies(
	...
	try {
		...
		throw launchGuard("worktree_dependency_install_failed", `${manager.name}:${String(error)}`);
	}
	if (result.exitCode !== 0) {
		throw launchGuard("worktree_dependency_install_failed", `${manager.name}:${result.stderr.toString().trim()}`);
	}
